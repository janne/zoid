...
### CLI changes:
  ...
  - Telegram MarkdownV2 sanitization should preserve existing escape pairs for reserved punctuation (for example `\(` and `\)`) instead of double-escaping them, so literal parentheses do not show with a leading backslash.
  - When Telegram rejects a MarkdownV2 chunk and delivery falls back to plain text (`parse_mode` omitted), strip MarkdownV2 escape backslashes (including repeated backslash runs before reserved punctuation) in that fallback chunk so users do not see artifacts like `\-`, `\(`, or `\\.`.
...
### Tool runtime changes:
  ...
  - In `lua_execute` tool-mode, remove `os`/`package`/`debug`/`require`/`dofile`/`loadfile`; expose `zoid.file(path)` metadata handles with `:read([max_bytes])/:write(content)/:delete()`, `zoid.dir(path)` metadata handles with `:list()/:create()/:remove()/:grep(pattern, [options])`, `zoid.uri(uri):get/post/put/delete`, `zoid.crypto.base64url_encode(data)`, `zoid.crypto.sign_rs256(private_key_pem, data, [encoding])`, `zoid.config():list/get/set/unset`, `zoid.jobs`, `zoid.browser.automate(options)`, `zoid.require(name)`, `zoid.json.decode`, `zoid.json.encode`, `zoid.time([table])`, `zoid.date([format[, epoch]])`, and `zoid.exit([code])`.
  - `zoid.crypto.sign_rs256` accepts PEM private keys (`BEGIN PRIVATE KEY` PKCS#8 or `BEGIN RSA PRIVATE KEY` PKCS#1), signs input with RSASSA-PKCS1-v1_5 SHA-256, and supports output encodings `base64url` (default), `base64`, `hex`, or `raw`.
  - `zoid.require(name)` only loads `.lua` files inside workspace root, supports explicit workspace/relative paths and extensionless module names, searches in `./?.lua`, `./?/init.lua`, `/lib/?.lua`, `/lib/?/init.lua`, `/scripts/?.lua`, and `/scripts/?/init.lua`, caches loaded modules per script execution, and rejects cyclic imports.
  ...
  - `zoid.json.decode` maps JSON values to Lua tables/scalars and maps JSON `null` to the sentinel `zoid.json.null`; `zoid.json.encode` serializes Lua values back to JSON and treats `zoid.json.null` as JSON `null`.
...
### Lua script examples (`workspace/scripts/*.lua`) changes:
  - Keep scripts compatible with the `zoid` API surface (`zoid.file`, `zoid.dir`, `zoid.uri`, `zoid.crypto`, `zoid.config`, `zoid.jobs`, `zoid.browser.automate`, `zoid.require`, `zoid.json`, `zoid.time`, `zoid.date`, `zoid.exit`, `zoid.eprint`) and do not rely on removed globals like `os`/`package`/`require`.
  ...
  - Workspace template module `workspace/lib/gcloud.lua` provides a Google Cloud REST helper; load it as `zoid.require("gcloud")`. It supports service-account JWT flow and `GCLOUD_ACCESS_TOKEN` fallback for runtimes without `zoid.crypto`.
