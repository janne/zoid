...
### CLI changes:
  ...
  - When Telegram rejects a MarkdownV2 chunk and delivery falls back to plain text (`parse_mode` omitted), strip MarkdownV2 escape backslashes from reserved punctuation in that fallback chunk so users do not see artifacts like `\-` or `\(`.
