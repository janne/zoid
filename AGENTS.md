...
### Tool runtime changes:
  ...
  - `http_get`/`http_post`/`http_put`/`http_delete` are direct HTTP(S) tools (no Lua script required); accepted input is a `uri` string plus optional `body` for `post`/`put`, and responses include `status` + `headers` + `body` with `ok` reflecting 2xx status.
